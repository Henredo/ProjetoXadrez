...
import chess.pieces.*; // Importa todas as peças (King, Rook, Pawn, etc.)
...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		if (movedPiece instanceof Pawn) {
			if ((movedPiece.getColor() == Color.WHITE && target.getRow() == 0) || (movedPiece.getColor() == Color.BLACK && target.getRow() == 7)) {
				...
				promoted = replacePromotedPiece("Q"); // Padrão vira Rainha
			}
		}
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		// #MOVIMENTO ESPECIAL: ROQUE (Desfazer)
		// Roque Pequeno
		if (p instanceof King && target.getColumn() == source.getColumn() + 2) {
			Position sourceT = new Position(source.getRow(), source.getColumn() + 3);
			Position targetT = new Position(source.getRow(), source.getColumn() + 1);
	...
	private void initialSetup() {
		...
		placeNewPiece('e', 1, new King(board, Color.WHITE, this)); // Passa 'this' para o Rei (Roque)
		placeNewPiece('f', 1, new Bishop(board, Color.WHITE));
		placeNewPiece('g', 1, new Knight(board, Color.WHITE));
		placeNewPiece('h', 1, new Rook(board, Color.WHITE));

		placeNewPiece('a', 2, new Pawn(board, Color.WHITE, this)); // Passa 'this' para o Peão (En Passant)
